...
**Video thumbnails use GStreamer frame extraction.** `extract_video_frame` builds a `uridecodebin → videoflip → videoconvert → appsink` pipeline. `videoflip video-direction=auto` automatically detects and applies rotation from all sources (MP4 tkhd matrix, H.264/H.265 SEI messages, GStreamer tags). The pipeline seeks to ~1s (or 10% of duration), pulls a single RGB frame as a `Pixbuf`. A semi-transparent play icon is composited onto the bottom-left corner so video tiles are visually distinguishable from photos in the grid. If GStreamer extraction fails (missing codec, corrupt file, timeout), the code falls back to `generate_video_placeholder`, which draws a dark-gray rectangle with a centered play triangle — the old synthetic placeholder. Both paths cache the result as JPEG.
