...
The prewarm pull offset follows the user's browsing position, not always the newest rows. Because the scrollbar can jump to any region instantly, the Photos grid calls `ThumbnailLoader::redirect_prewarm_to_offset(target_start)` when a virtual DB page lands, moving the prewarm pull start to the landed window so off-screen tiles around the current position warm next (ahead of irrelevant newest-first batches). Visible tiles still take `TIER_BOOST` (highest priority); this redirect only decides where the lower-priority off-screen prewarm work happens. It does not change the pull model, DESC order, tier, or throttle.
