...
Startup must not wait for the full filesystem scan before showing Photos. `app::initialize` loads only the first 200 live DB rows for the initial grid, then runs startup scanning, trash reconciliation, and bounded DB pagination in background work. The GTK-facing `media_list` is capped by `apply_to_media_list::ui_media_list_cap()` (configurable via `settings.json`, default 200 newest rows); the database remains the full source of truth for scans, search, album counts, and future pagination. Startup scan batches are ignored by the GTK model once that cap is filled, preventing repeated visible grid rebuilds while the database scan continues. The startup scan uses `LocalBackend::scan_and_upsert_dir_notify`, preserving the `(uri, file_mtime, file_size)` unchanged-file short-circuit while sending each actually upserted live `MediaItem` through `MediaChangeNotifier::upserted` so the existing GTK consumer can merge it into the shared `media_list` without letting UI memory grow with the full library.
