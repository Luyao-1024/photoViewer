...
| `album:select_row`, `album:open_idle`, `album:open` (+ `album:already_visible_check`/`pop`/`load`/`store`/`page_build`/`bind_page`/`push`), `album:backfill_schedule`, `album:backfill_fetch` | `ui/window/navigation.rs` owns sidebar album selection → idle handoff → album open/page-build phases, while `ui/window/albums.rs` owns the background backfill helpers |
