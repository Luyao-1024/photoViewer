...
| `thumb:generate` (nests `thumb:pb_decode`/`pb_scale`/`pb_save`), `thumb:process`, `thumb:set_prewarm_thumbnail_size` | `core/thumbnails.rs` per-image decode (decode/scale/save sub-phases), worker per-item envelope, and background-prewarm size changes. The per-thumbnail worker/generate/decode/scale/save spans are debug-level so large libraries do not produce default INFO trace volume per image. |
...
| `grid:load_metadata`, `grid:set_active`, `grid:rebuild_immediately`, `grid:scheduled_rebuild`, `grid:rebuild`, `grid:clear_content`, `grid:extract_items`, `grid:group_sections`, `grid:build_sections`, `grid:page_query` (+ `grid:db_page`), `grid:thumb_request`, `grid:reprioritize`, `grid:try_virtual_page`, `grid:expand_render_limit`; `ui:apply_upserted_batch`, `ui:apply_absent_insertions`, `ui:apply_targeted_upserts` | `ui/media_grid.rs` grid activation, rebuild trigger, coarse rebuild phases, virtual-scroll page load, per-tile thumb request, and the **fast-scroll hot path**: debounced visible-tile reprioritize, virtual-page retarget (no synchronous rebuild — marks retargets only), and near-bottom render-limit expansion. `ui/apply_to_media_list.rs` list-apply spans cover batch upsert/insert paths. High-frequency `grid:thumb_request` / `grid:reprioritize` and batch list-apply spans are debug-level so default INFO logs stay readable. |
