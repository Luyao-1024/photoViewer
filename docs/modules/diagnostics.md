...
| `thumb:generate` (nests `thumb:pb_decode`/`pb_scale`/`pb_save`), `thumb:process`, `thumb:set_prewarm_thumbnail_size` | `core/thumbnails/decode.rs` owns per-image decode plus decode/scale/save sub-phases, while `core/thumbnails.rs` and `core/thumbnails/queue.rs` own the background-prewarm size changes and worker per-item envelope. The per-thumbnail worker/generate/decode/scale/save spans are debug-level so large libraries do not produce default INFO trace volume per image. |
...
| `vgrid:scroll_range`, `vgrid:db_range`, `vgrid:landing`, `vgrid:replace_ready_range`, `vgrid:evict`, `vgrid:emit_replacements`, `vgrid:thumbnail_flush`, `tile:paint` | `ui/virtual_media_grid.rs` and its model/factory trace the Photos GridView scroll path: main-thread viewport/range coordination, off-thread repository range query, main-thread model landing/replacement, thumbnail-request batch issue, and bind-to-paint latency. These high-frequency spans are debug-level. |
| `grid:load_metadata`, `grid:set_active`, `grid:rebuild_immediately`, `grid:scheduled_rebuild`, `grid:rebuild`, `grid:clear_content`, `grid:extract_items`, `grid:group_sections`, `grid:build_sections`, `grid:page_query` (+ `grid:db_page`), `grid:thumb_request`, `grid:reprioritize`, `grid:try_virtual_page`, `grid:expand_render_limit`; `ui:apply_upserted_batch`, `ui:apply_absent_insertions`, `ui:apply_targeted_upserts` | `ui/media_grid.rs` owns metadata load and active-state switching, `ui/media_grid/loading.rs` owns rebuild scheduling, `ui/media_grid/viewport.rs` owns virtual-scroll page load / reprioritize / render-limit expansion, `ui/media_grid/updates.rs` owns the coarse rebuild phases, and `ui/media_grid/render.rs` owns per-tile thumb requests. `ui/apply_to_media_list.rs` list-apply spans cover batch upsert/insert paths. The **fast-scroll hot path** still means the same debounced visible-tile reprioritize, virtual-page retarget (no synchronous rebuild — marks retargets only), and near-bottom render-limit expansion. High-frequency `grid:thumb_request` / `grid:reprioritize` and batch list-apply spans are debug-level so default INFO logs stay readable. |
...
| `sidebar:rebuild_album_rows`, `sidebar:apply_album_rows`, `sidebar:apply_album_snapshot`, `sidebar:model_replace`, `sidebar:model_update`, `sidebar:factory_bind` | `ui/window/sidebar.rs` sidebar album snapshot application, full model replacement for order changes, changed-item updates, and per-realized-row binding |
...
For **fast-scroll stalls on the Photos page** ("thumbnails freeze / stop loading while flinging"), launch with `PHOTOVIEWER_CHROME_TRACE=1`, reproduce the fast scroll, close the app normally, then inspect `<cache-dir>/logs/trace.json` in Perfetto. The expensive or high-volume parts of this path are deliberately debug-level diagnostics; use a debug build, or a release build compiled with `release_max_level_debug`, when their fields are needed:

- `vgrid:scroll_range` — the synchronous main-thread work performed for each vertical-adjustment update. Its fields identify the viewport range, overscan target, and whether the coordinator started, coalesced, or skipped a query. A long/dense run indicates the scroll callback itself is consuming frame time.
- `vgrid:db_range` → `vgrid:landing` — separates blocking repository-query latency from applying the returned range to the GTK model. `vgrid:replace_ready_range`, `vgrid:evict`, and `vgrid:emit_replacements` subdivide a costly landing; `vgrid:thumbnail_flush` shows how many thumbnail requests were issued in one main-loop turn; `tile:paint.elapsed_ms` is bind-to-paint latency.
...
- `grid:thumb_request` — wraps per-tile thumbnail request completion as debug-only tracing. Enter it only after the async worker returns; do not hold the span guard across `await`, or unrelated later logs can inherit nested request/reprioritize prefixes.
