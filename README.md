...
**0.9.0 release candidate**: Full browsing, thumbnails, viewer, albums, trash,
editor, video playback, motion photo playback, theme preferences, and
Flathub-oriented packaging metadata.
...
当前 Ubuntu 24.04 系统 GTK4 停留在 4.14.x。若要在不替换系统 GTK 的前提下使用更新的 GTK 运行时，使用项目根目录的 `io.github.luyao_1024.photoviewer.yml` 构建 Flatpak。该清单固定到 `org.gnome.Platform//50`，并通过 Rust SDK 扩展在沙箱内构建 release 二进制。
...
flatpak run io.github.luyao_1024.photoviewer
```

说明：

- Flatpak 清单使用 `cargo-sources.json` 固定 Rust crate 源，构建阶段使用 `cargo build --release --locked` 保持依赖版本与 `Cargo.lock` 一致。
...
```bash
flatpak info io.github.luyao_1024.photoviewer
```
...
2. 或写入配置文件（持久生效）：
   ```bash
   mkdir -p ~/.config/io.github.luyao_1024.photoviewer
   cp config/i18n.en.example.json ~/.config/io.github.luyao_1024.photoviewer/i18n.json
   ```
