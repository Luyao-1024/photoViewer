...
use crate::core::backend::local::{scan_root_is_available, LocalBackend};
...
pub async fn scan_and_aggregate(pool: &DbPool, roots: &[PathBuf]) -> Result<()> {
    ...
    tokio::task::spawn_blocking(move || -> Result<()> {
        ...
        for root in &roots {
            if !scan_root_is_available(root) {
                tracing::warn!(
        ...
        let removed =
            backend.prune_missing_live_media_under_roots(&scanned_roots, &excluded_roots)?;
        if !removed.is_empty() {
            tracing::info!(
                target: crate::core::log_targets::STORAGE,
...
fn scan_and_aggregate_with_actor_blocking(
    ...
    let missing =
        backend.collect_missing_live_media_under_roots(&scanned_roots, &excluded_roots)?;
    let removed = match db_actor
        .execute_blocking_in_trace(trace.clone(), DbCommand::PruneMissingLiveRows { missing })?
    {
...
fn scan_and_aggregate_with_notifier_blocking(
    ...
    let removed = backend.prune_missing_live_media_under_roots(&scanned_roots, &excluded_roots)?;
    if !removed.is_empty() {
        tracing::info!(
            target: crate::core::log_targets::STORAGE,
            "STARTUP_SCAN_PRUNE removed_missing_live={}",
