...
const THUMB_EDGE_INSET: f64 = 24.0;
/// GTK can briefly report 2px-wide children immediately after rebuilding the
/// carousel. Treat those allocations as unstable and retry on a later frame
/// instead of applying a large, wrong transform.
const THUMB_MIN_STABLE_ALLOC_WIDTH: f64 = (THUMB_MIN_WIDTH as f64) * 0.75;
...
const THUMB_SCROLL_ANIMATION_MS: f64 = 140.0;
...
fn compute_thumb_animated_scroll_value(start: f64, target: f64, progress: f64) -> f64 {
...
fn thumb_width_for_media_dimensions(width: Option<u32>, height: Option<u32>) -> i32 {
...
/// Once the live widget cap is reached, extending slides the bounded window
/// instead of growing it, so the carousel can keep feeding later/earlier
/// thumbnails without making GTK hold the whole album.
...
enum ThumbWindowUpdateKind {
    AppendRight,
    PrependLeft,
    Rebuild,
}

fn classify_thumb_window_update(
    old_start: u32,
    old_end: u32,
    new_start: u32,
    new_end: u32,
