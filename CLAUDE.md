...
### Liquid Glass styling is dual-mode and toggleable
Liquid Glass is a user setting (Settings → 外观/Appearance; default on, opt-out; persisted in `settings.json` via `core::prefs`). The glass CSS in `src/ui/grid_css.rs` is split into `BASE_CSS` (shared layout/state), `LIQUID_GLASS_MATERIAL_CSS` (blur+saturate+brightness+highlights+heavy shadows), `PLAIN_GLASS_MATERIAL_CSS` (plain semi-transparent, **no** `backdrop-filter`, opaque buttons), and `A11Y_CSS` (shared `@media` fallbacks). `build_css(bool)` picks the material block; `grid_css::install()` reads the pref at startup, `grid_css::reapply(bool)` swaps the display `CssProvider` live on toggle (no restart). **When adding a new glass surface you MUST add it to BOTH material blocks** (or reuse an existing `.glass-base`/`.glass-raised`/`.glass-header`/`.glass-menu`/`.glass-alert-dialog`/`.viewer-details-panel` class) so it looks right in both modes; never put `backdrop-filter` in `BASE_CSS`. Full convention + the unit tests that guard the split are in `docs/liquid-glass.md`. `backdrop-filter` only renders under the GNOME 50 (Flatpak) runtime — verify visually with `./run-flatpak.sh`.
